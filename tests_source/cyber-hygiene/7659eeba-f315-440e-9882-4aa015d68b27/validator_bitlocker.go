...
const validatorName = "bitlocker"

func main() {
	fmt.Println("[validator-bitlocker] Running BitLocker Cloud Escrow checks...")
	...
	result := RunBitLockerEscrowChecks()
	...
	if result.IsCompliant {
		fmt.Printf("[validator-bitlocker] COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
		os.Exit(126)
	}
	fmt.Printf("[validator-bitlocker] NON-COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
	os.Exit(101)
}
