...
const validatorName = "whfb"

func main() {
	fmt.Println("[validator-whfb] Running Windows Hello for Business checks...")
	...
	if result.IsCompliant {
		fmt.Printf("[validator-whfb] COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
		os.Exit(126)
	}
	fmt.Printf("[validator-whfb] NON-COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
	os.Exit(101)
}
