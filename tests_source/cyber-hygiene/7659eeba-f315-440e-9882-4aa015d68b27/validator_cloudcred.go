...
const validatorName = "cloudcred"

func main() {
	fmt.Println("[validator-cloudcred] Running Cloud Credential Protection checks...")
	...
	result := RunCloudCredentialChecks()
	...
	if result.IsCompliant {
		fmt.Printf("[validator-cloudcred] COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
		os.Exit(126)
	}
	fmt.Printf("[validator-cloudcred] NON-COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
	os.Exit(101)
}
