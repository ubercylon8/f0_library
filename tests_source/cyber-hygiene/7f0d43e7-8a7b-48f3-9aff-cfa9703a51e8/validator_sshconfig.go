...
// validator_sshconfig.go - standalone validator binary for SSH configuration checks.
...
const validatorName = "sshconfig"

func main() {
	fmt.Println("[validator-sshconfig] Running SSH configuration checks...")
	...
	if result.IsCompliant {
		fmt.Printf("[validator-sshconfig] COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
		os.Exit(126)
	}
	fmt.Printf("[validator-sshconfig] NON-COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
	os.Exit(101)
}
