...
// validator_filesystem.go - standalone validator binary for filesystem security checks.
...
const validatorName = "filesystem"

func main() {
	fmt.Println("[validator-filesystem] Running filesystem security checks...")
	...
	if result.IsCompliant {
		fmt.Printf("[validator-filesystem] COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
		os.Exit(126)
	}
	fmt.Printf("[validator-filesystem] NON-COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
	os.Exit(101)
}
