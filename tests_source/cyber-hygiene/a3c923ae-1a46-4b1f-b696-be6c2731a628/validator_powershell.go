...
const validatorName = "powershell"

func main() {
	fmt.Println("[validator-powershell] Running PowerShell Security checks...")
	...
	if result.IsCompliant {
		fmt.Printf("[validator-powershell] COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
		os.Exit(126)
	}
	fmt.Printf("[validator-powershell] NON-COMPLIANT (%d/%d passed)\n", result.PassedCount, result.TotalChecks)
	os.Exit(101)
}
