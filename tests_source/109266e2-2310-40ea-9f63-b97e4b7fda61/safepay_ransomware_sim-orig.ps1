...
function Test-DiskSpace {
    ...
    try {
        ...
        return $freeSpaceGB -ge 2.0  # Require at least 2GB free
    } catch {
...
function New-CorporateFileTree {
    ...
    Write-Host "[*] Creating realistic corporate directory structure..." -ForegroundColor Cyan
...
function New-RealisticCorporateFiles {
    ...
    Write-Host "[*] Generating realistic corporate files..." -ForegroundColor Cyan
    ...
    foreach ($directory in $directories) {
        if ($totalFiles -ge $maxFiles) {
            Write-Host "[!] Reached maximum file limit ($maxFiles) for safety" -ForegroundColor Yellow
            break
        }
    ...
    Write-Host "[+] File generation complete. Total files created: $totalFiles" -ForegroundColor Green
    return $totalFiles
}
...
function Invoke-MultiPhaseCompression {
    ...
    Write-Host "[*] Starting multi-phase compression operations..." -ForegroundColor Cyan
    
    if (-not (Test-Path $WinRARPath)) {
        Write-Host "[!] WinRAR not found at: $WinRARPath" -ForegroundColor Red
        return $false
    }
    
    # Phase 1: Department-level archives
    Write-Host "[*] Phase 1: Creating department-level archives..." -ForegroundColor Yellow
    $departments = @("Finance", "HR", "Legal", "IT", "Sales", "Executive")
    ...
    foreach ($dept in $departments) {
        ...
        if ($deptPath) {
            ...
            try {
                Write-Host "[+] Archiving $dept department..." -ForegroundColor Green
                $process = Start-Process -FilePath $WinRARPath -ArgumentList $rarArgs -Wait -PassThru -NoNewWindow
                if ($process.ExitCode -eq 0) {
                    Write-Host "[+] $dept archive created successfully" -ForegroundColor Green
                } else {
    ...
    Write-Host "[*] Phase 2: Creating location-based archives..." -ForegroundColor Yellow
    ...
    foreach ($location in $locations) {
        ...
        if (Test-Path $locationPath) {
            ...
            $jobs += $job
            Write-Host "[+] Started background compression for $location" -ForegroundColor Green
        }
    }
    
    # Wait for all compression jobs to complete
    if ($jobs.Count -gt 0) {
        Write-Host "[*] Waiting for parallel compression jobs to complete..." -ForegroundColor Cyan
        $jobs | Wait-Job | ForEach-Object {
            $result = Receive-Job $_
            if ($result.Success) {
                Write-Host "[+] $($result.Location) compression completed successfully" -ForegroundColor Green
            } else {
                Write-Host "[!] $($result.Location) compression failed (Exit Code: $($result.ExitCode))" -ForegroundColor Red
            }
            Remove-Job $_
        }
    }
    ...
    Write-Host "[*] Phase 3: Creating master exfiltration archive..." -ForegroundColor Yellow
    ...
    try {
        Write-Host "[+] Creating master exfiltration archive..." -ForegroundColor Green
        $process = Start-Process -FilePath $WinRARPath -ArgumentList $masterArgs -Wait -PassThru -NoNewWindow
        if ($process.ExitCode -eq 0) {
            Write-Host "[+] Master exfiltration archive created successfully" -ForegroundColor Green
        } else {
            Write-Host "[!] Master archive creation failed with exit code: $($process.ExitCode)" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "[!] Failed to create master archive: $_" -ForegroundColor Red
    }
...
function Remove-OriginalFiles {
    param(
        [string]$BasePath,
        [array]$ExcludePatterns = @("*.rar", "*.safepay", "readme_safepay.txt")
    )
    
    Write-Host "[*] Starting mass deletion of original files..." -ForegroundColor Yellow
    ...
    for ($i = 0; $i -lt $filesToDelete.Count; $i += $batchSize) {
        ...
        Write-Host "[*] Secure deletion batch $(([Math]::Floor($i / $batchSize)) + 1)..." -ForegroundColor Yellow
    ...
    do {
        ...
        foreach ($dir in $emptyDirs) {
            try {
                ...
                Write-Host "[!] Failed to remove directory $($dir.Name): $_" -ForegroundColor Red
            }
        }
    } while ($emptyDirs.Count -gt 0)
    
    Write-Host "[+] Mass deletion complete. Deleted $deletedCount files" -ForegroundColor Green
    return $deletedCount
}
...
if (-not (Test-DiskSpace)) {
    Write-Host "[!] Insufficient disk space. Minimum 2GB required." -ForegroundColor Red
    exit 1
}
...
Write-Host "[*] Waiting 3 seconds before initiating mass deletion..." -ForegroundColor Cyan
Start-Sleep -Seconds 3
$deletedCount = Remove-OriginalFiles -BasePath $targetDir

# Simulate file encryption on any remaining files
Write-Host "[*] Simulating final encryption phase on remaining files..." -ForegroundColor Cyan
...
if ($filesToEncrypt.Count -gt 0) {
    Write-Host "[*] Found $($filesToEncrypt.Count) files remaining for encryption..." -ForegroundColor Yellow
    ...
    Write-Host "[*] No files remaining for encryption (mass deletion completed)" -ForegroundColor Cyan
}
...
Write-Host "[*] Generating attack summary statistics..." -ForegroundColor Cyan
$encryptedFiles = Get-ChildItem -Path $targetDir -Filter "*.safepay" -Recurse -File | Measure-Object | Select-Object -ExpandProperty Count
$archiveFiles = Get-ChildItem -Path $targetDir -Filter "*.rar" -Recurse -File | Measure-Object | Select-Object -ExpandProperty Count

Write-Host "[+] ATTACK SUMMARY:" -ForegroundColor Green
Write-Host "    Files Created: $filesCreated" -ForegroundColor Green
Write-Host "    Files Deleted: $deletedCount" -ForegroundColor Green  
Write-Host "    Files Encrypted: $encryptedFiles" -ForegroundColor Green
Write-Host "    Archive Files: $archiveFiles" -ForegroundColor Green
Write-Host "    Target Location: $targetDir" -ForegroundColor Green
