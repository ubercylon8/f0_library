...
# Stage binaries to build (technique ID : source file without extension)
declare -a STAGES=(
...
echo "  Test: TA453 NICECURL VBScript Backdoor Detection"
echo ""
...
if [ "$GOOS" = "windows" ] && command -v osslsigncode &> /dev/null; then
    ...
    print_warning "Skipping signature verification (not applicable for ${GOOS} or osslsigncode not available)"
fi
...
echo "  Test Name:        TA453 NICECURL VBScript Backdoor Detection"
...
echo "  Test will extract and execute 3 stages in NICECURL killchain order"
echo ""
