...
# KslKatz LSASS Credential Dumping Framework (BYOVD PPL Bypass) — 4 stages
#
# CRITICAL BUILD SEQUENCE (modern 8-step pattern):
# 1. Build unsigned stage binaries
# 2. (no cleanup utility for this test)
# 3. Sign stage binaries (BEFORE embedding)
# 4. Verify signatures
# 5. Compress signed binaries with gzip (reduces orchestrator size ~35%)
...
print_header "Multi-Stage Test Build: KslKatz LSASS BYOVD PPL Bypass"
...
# Step 2: (no separate cleanup utility for this test)
print_step "2/8" "No cleanup utility for this test — stages clean up their own sandbox artifacts."
...
# Step 5: Compress signed binaries with gzip (reduces orchestrator size ~35%)
...
echo ""
echo "  Test UUID:        ${TEST_UUID}"
echo "  Test Name:        KslKatz LSASS Credential Dumping Framework (BYOVD PPL Bypass)"
echo "  Stages Built:     ${stage_count}"
echo "  Final Binary:     ${BUILD_DIR}/${TEST_UUID}.exe"
echo "  Binary Size:      ${main_size_final} (${main_size_bytes} bytes)"
