# References & Sources

## Primary Source

*Retroactively reconstructed from existing test documentation.*

| Field | Value |
|-------|-------|
| **Title** | Microsoft Threat Intelligence - Peach Sandstorm |
| **Author/Organization** | Microsoft |
| **Date Published** | 2026-03-08 |
| **Document Type** | blog-post |
| **URL** | https://www.microsoft.com/security/blog/tag/peach-sandstorm/ |

## Supporting Resources

Additional threat intelligence, incident reports, and TTP analyses referenced during test development.

| # | Title | Type | URL |
|---|-------|------|-----|
| 1 | FBI/CISA Advisory AA24-241a | Government Advisory | https://www.cisa.gov/news-events/cybersecurity-advisories |
| 2 | Tickler Backdoor Analysis - DLL Sideloading via Azure Infrastructure | Blog Post | https://www.microsoft.com/en-us/security/blog/2024/08/28/peach-sandstorm-deploys-new-custom-tickler-malware-in-long-running-intelligence-gathering-operations/ |

## MITRE ATT&CK References

| Technique | Name | URL |
|-----------|------|-----|
| T1566.001 | T1566.001 | https://attack.mitre.org/techniques/T1566/001/ |
| T1574.002 | DLL Side-Loading | https://attack.mitre.org/techniques/T1574/002/ |
| T1547.001 | Registry Run Keys / Startup Folder | https://attack.mitre.org/techniques/T1547/001/ |
| T1053.005 | Scheduled Task | https://attack.mitre.org/techniques/T1053/005/ |
| T1036 | Masquerading | https://attack.mitre.org/techniques/T1036/ |
| T1071.001 | Web Protocols | https://attack.mitre.org/techniques/T1071/001/ |

## Related Advisories & News

- [FBI/CISA Advisory AA24-241a](https://www.cisa.gov/news-events/cybersecurity-advisories)
- [MITRE ATT&CK - APT33](https://attack.mitre.org/groups/G0064/)
