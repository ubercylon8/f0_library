...
# Multi-stage build script (3 stages + orchestrator)
#
...
# Stage definitions: "TECHNIQUE_TAG:SOURCE_FILE"
# Each stage binary: ${TEST_UUID}-${TECHNIQUE_TAG}.exe
declare -a STAGES=(
    "T1083:stage-T1083"
    "T1562.001-lock:stage-T1562.001-lock"
    "T1562.001-svcnotify:stage-T1562.001-svcnotify"
)
...
sign_binary() {
    ...
    if [ "$SIGN_MODE" = "projectachilles" ]; then
        ...
        print_warning "codesign utility not found — falling back to single sign"
        ...
        if [ ! -f "$SIGN_PASS_FILE" ]; then
            print_warning "Cert password file not found — skipping"
            return 0
        fi
        local password
        password=$(tr -d '\n\r' < "$SIGN_PASS_FILE")
...
print_header "UnDefend Multi-Stage Build"
...
# Step 4: Compress with gzip (mandatory — never use UPX or runtime packers)
print_step "4/7" "Compressing signed binaries with gzip..."
for stage in "${STAGES[@]}"; do
    IFS=':' read -r tag _ <<< "$stage"
    binary="${TEST_UUID}-${tag}.exe"
...
print_step "5/7" "Building orchestrator (embedding compressed stages)..."
mkdir -p "${BUILD_DIR}"
...
echo ""
echo "  Test UUID:        ${TEST_UUID}"
echo "  Test Name:        UnDefend - Defender Signature/Engine Update DoS via File-Lock Race"
...
for stage in "${STAGES[@]}"; do
    IFS=':' read -r tag source <<< "$stage"
    echo "  - ${TEST_UUID}-${tag}.exe (${source}.go)"
done
echo ""
print_success "UnDefend simulation ready for deployment."
