...
| **2c** | Telemetry Signal Quality (0–2.0) | **1.6** (lab-verified — cap lifted) | Criterion 1 (signal richness) ✅ — every stage emits an identifiable signal in its native telemetry surface (image/file-drop events, registry reads, DNS/TLS egress, AES-decrypt of appended ICO data, rapid multi-browser file reads). Criterion 2 (sensor mapping) ✅ — see Detection Opportunity Audit below; the detection suite is cross-referenced per primitive. Criterion 3 (rule-artifact validity) **mostly met** — all 5 supported rule formats are present (`_detections.kql`, `_rules.yar`, `_sigma_rules.yml`, `_elastic_rules.ndjson`, `_dr_rules.yaml`) and well-formed on manual review; the only gap is they were not run through an automated linter/YARA compiler. Criterion 4 (lab execution) ✅ — **detonated on the `win` lab (Win11 Pro, Defender RTP ON) 2026-07-07** (see Lab Detonation Results below): all 4 stages reached and completed (exit 101), and Stage 1's side-load image-load was positively confirmed in the captured child output (`LOADED d3dcompiler_47.dll`, `LOADED ffmpeg.dll`), with all stage binaries and planted DLLs surviving on disk (no quarantine). This clears the v2.1 no-lab-evidence hard cap. Held at 1.6 rather than 2.0 because (a) Defender produced **no** detection signal — a valid finding about the endpoint, but the run did not positively confirm that any *custom* detection rule (our KQL/Sigma/EQL/YARA/D&R) actually fires against the emitted telemetry, since they were not loaded into a sensor; and (b) the rule artifacts were not machine-linted/compiled. |
...
| | **Total** | **7.9 / 10** | Realism: 5.3 / 7.0 (API 2.0, Identifier 1.2, Telemetry 1.6, Exec-Context 0.5). Structure: 2.6 / 3.0. **Good** tier (6.0–7.9). **Lab-detonated on the `win` lab 2026-07-07** (clean exit 101, all 4 stages reached, Stage-1 image-load confirmed) — meets the v2.1 ship gates (Realism ≥ 5.0, Total ≥ 7.5). Remaining upside toward a higher 2c: load the custom detection rules into a sensor and confirm they fire against the emitted telemetry. |

### Lab Detonation Results (2026-07-07)

Detonated on the `win` lab — Windows 11 Pro (build 10.0.26200), Windows Defender real-time protection **ON** (`RealTimeProtectionEnabled=True`, engine `1.1.26060.3008`). Ran the signed build SHA1 `19dba609044598bd1cfdf99f6bae99413d3abca2` as `jimx` (admin); no snapshot (non-destructive test). Behavior is identical for the current committed build — only the `TestMetadata.Score` constant changed after this run (7.6 → 7.9), not the executable logic.

| Observation | Result |
|---|---|
| Overall exit code | **101 `Endpoint.Unprotected`** (corroborated by console, `bundle_results.json`, `test_execution_log.json`) |
| Stages reached | **All 4** (T1574.002, T1497, T1027.003, T1555.003) — each `status: success`, none skipped/blocked |
| Stage 1 side-load | **Image-load confirmed** — child output: `LOADED d3dcompiler_47.dll`, `LOADED ffmpeg.dll` (0 `LOADFAIL`) |
| Quarantine | **None** — all 4 stage binaries + planted DLLs survived on disk post-run |
| Defender reaction | **None** — no `Get-MpThreatDetection` entry for this run; zero Defender Operational events (1116/1117/config) in the execution window |
| Classification integrity | **Correct** — 101 truthfully reflects "all stages succeeded, no protection acted"; no block code manufactured (Rule 8 honored) |

**Interpretation:** on a protected host, the full 3CX cascading-supply-chain chain executed end-to-end undetected — a valid, meaningful *unprotected* finding about the endpoint, not a test defect. The run positively validated that the strengthened Stage 1 emits a genuine side-load image-load.
...
- **~~Confirm Stage 1's DLL-load telemetry in a lab run.~~ ✅ DONE (2026-07-07).** Stage 1 forces a real load — the signed `3CXDesktopApp.exe` copy calls `syscall.LoadLibrary` on the planted `d3dcompiler_47.dll`/`ffmpeg.dll` from its own directory. Confirmed on the `win` lab: the child `--sideload-host` process reported `LOADED d3dcompiler_47.dll` and `LOADED ffmpeg.dll` (zero `LOADFAIL`), so the image-load genuinely occurs. Criterion 4 cleared; 2c cap lifted. *Remaining upside:* load the custom detection rules into a sensor and confirm `DeviceImageLoadEvents`/Sysmon EID 7 (and the other stage rules) actually fire against the captured telemetry — that would take 2c toward 2.0. A deeper future refinement is a host with a static import-table dependency (requires a mingw/cgo toolchain not currently available) for passive search-order resolution instead of an explicit `LoadLibrary`.
