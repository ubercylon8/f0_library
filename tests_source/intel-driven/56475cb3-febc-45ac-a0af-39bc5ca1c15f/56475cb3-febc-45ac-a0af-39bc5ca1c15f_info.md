...
- **Stage 1 (T1574.002)** — the "trusted signed vendor app" role is played by our own F0RT1KA-signed orchestrator, which stages a benign Microsoft-signed binary (`notepad.exe`, renamed `3CXDesktopApp.exe`) alongside benign marker DLLs named after the real side-load companions (`d3dcompiler_47.dll`, `ffmpeg.dll`) in a user-writable `ARTIFACT_DIR` app path, then launches it — reproducing the file-layout and process-launch primitive that gives a signed parent access to non-system DLLs from its own directory.
...
|---|---|---|---|
| **1** | Safety Gate | **PASS** | All writes confined to `ARTIFACT_DIR` (`c:\Users\fortika-test`) or `LOG_DIR` (`C:\F0`). No write/delete/lock/reparse intent against real system targets — the only real-system read is `os.ReadFile` on `C:\Windows\System32\notepad.exe` (read-only). No COM activation, service creation, driver load, or token/privilege manipulation. Network egress is real but benign: DNS + HTTPS GET to `raw.githubusercontent.com` (a real, legitimate GitHub CDN host, not attacker infrastructure), response body discarded and never used as a payload or C2 channel — consistent with this repo's established pattern for T1071.001 stages (see e.g. `e5472cd5-c799-4b07-b455-8c02665ca4cf`). Stage 1/4 decoys are cleaned up via `os.RemoveAll` on the success path; **note** — on an error/quarantine exit the decoy cleanup step is skipped (flagged below, does not disqualify: decoys never leave the sandboxed `ARTIFACT_DIR` and contain only benign sentinel content). |
| **2a** | API Fidelity (0–2.5) | **1.7** | Stage 2 (registry `QUERY_VALUE` reads + `GetTickCount64`) and Stage 3 (real `net.LookupHost` + `http.Client.Do` + manual `ICONDIR`/`ICONDIRENTRY` binary parsing + `crypto/aes`/`crypto/cipher` CBC) closely match the documented PoC API surface. Stage 1 is the weakest link: it places the companion DLLs and launches the signed stand-in, but `notepad.exe` has no import-table dependency on `d3dcompiler_47.dll`/`ffmpeg.dll`, so the test is **not guaranteed to trigger an actual `LoadLibrary`/image-load event** for those filenames the way the real trojanized `3CXDesktopApp.exe` did (which genuinely imported `d3dcompiler_47.dll`). Stage 4 abstracts the real ICONIC mechanism (SQLite query against `Login Data` + DPAPI `CryptUnprotectData` decrypt) down to a plain file copy. Both gaps are documented under Improvement Opportunities. |
...
| | **Total** | **7.3 / 10** | Realism: 4.7 / 7.0. Structure: 2.6 / 3.0. **Good** tier (6.0–7.9). Below the v2.1 ship gates (Realism ≥ 5.0, Total ≥ 7.5) — see Improvement Opportunities for the concrete path to ship-readiness. |
...
2. **Stage 1 — DLL Side-Loading (T1574.002)**: Copies `C:\Windows\System32\notepad.exe` to `ARTIFACT_DIR\3CXDesktopApp\app\3CXDesktopApp.exe` (the signed-parent stand-in), drops benign marker files named `d3dcompiler_47.dll` and `ffmpeg.dll` (valid `MZ` header + F0RT1KA sentinel content) beside it, waits 3 s for a protection reaction, then launches the stand-in from that directory (`cmd.Dir = appDir`) so the OS DLL search order would resolve the companions from the app directory if referenced. Waits 4 s, kills the process, checks for quarantine, then cleans up the staged directory.
...
- **Confirm Stage 1's DLL-load telemetry in a lab run.** `notepad.exe` (the signed stand-in) has no import-table dependency on `d3dcompiler_47.dll`/`ffmpeg.dll`, so placing those files beside it and launching it may not trigger an actual `LoadLibrary`/image-load event the way the real trojanized `3CXDesktopApp.exe` did (which genuinely imported `d3dcompiler_47.dll`). Verify with a lab run whether `DeviceImageLoadEvents` fires for these filenames under the current stand-in, and if not, consider a stand-in binary confirmed to dynamically probe for one of these DLL names.
