...
# 3CX 3CXDesktopApp Cascading Supply-Chain Compromise — 4 stages
#
# CRITICAL BUILD SEQUENCE (modern 8-step pattern):
...
# 8. Calculate SHA1 hash + report size tier
#
...
sign_binary() {
    ...
    if [ "$SIGN_MODE" = "projectachilles" ]; then
        ...
        print_warning "codesign utility not found — skipping org signing for ${binary_name}"
        return 0
...
print_header "Multi-Stage Test Build: 3CX Cascading Supply-Chain Compromise"
...
# Step 7: Cleanup temporary stage binaries (.exe and .exe.gz)
...
# Step 8: Calculate SHA1 hash + size tier
print_step "8/8" "Calculating hash and size tier..."
main_hash=$(sha1sum "${main_binary}" | awk '{print $1}')
main_bytes=$(stat -c%s "${main_binary}" 2>/dev/null || stat -f%z "${main_binary}" 2>/dev/null)
main_mb=$(awk "BEGIN{printf \"%.2f\", ${main_bytes}/1048576}")
main_size_final=$(ls -lh "${main_binary}" | awk '{print $5}')

# Size budget tiers (CLAUDE.md): Green <=10MB, Yellow 10-25MB, Red 25-50MB, Forbidden >50MB
tier="GREEN"
if awk "BEGIN{exit !(${main_bytes} > 52428800)}"; then
    tier="FORBIDDEN (>50MB — DO NOT SHIP)"
elif awk "BEGIN{exit !(${main_bytes} > 26214400)}"; then
    tier="RED (25-50MB)"
elif awk "BEGIN{exit !(${main_bytes} > 10485760)}"; then
    tier="YELLOW (10-25MB)"
fi
...
echo ""
echo "  Test UUID:        ${TEST_UUID}"
echo "  Test Name:        3CX 3CXDesktopApp Cascading Supply-Chain Compromise"
echo "  Stages Built:     ${stage_count}"
echo "  Final Binary:     ${BUILD_DIR}/${TEST_UUID}.exe"
echo "  Binary Size:      ${main_size_final} (${main_mb} MB)"
echo "  Size Tier:        ${tier}"
