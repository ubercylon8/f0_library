...
/*
ID: 414a4c61-019f-48ba-934d-d5e91a29a878
NAME: UNK_RobotDreams Rust Backdoor Execution Chain
TECHNIQUES: T1204.002, T1059.001, T1105, T1071.001, T1573.001, T1036.005
...
SUBCATEGORY: apt
TAGS: powershell, domain-fronting, azure, cloud-c2, rust-backdoor, pdf-lure, unk-robotdreams
SOURCE_URL: https://gbhackers.com/iran-war-bait/
UNIT: response
CREATED: 2026-03-24
...
const (
	...
	TEST_NAME = "UNK_RobotDreams Rust Backdoor Execution Chain"
)
...
//go:embed 414a4c61-019f-48ba-934d-d5e91a29a878-T1204.002.exe.gz
var stage1Compressed []byte

//go:embed 414a4c61-019f-48ba-934d-d5e91a29a878-T1059.001.exe.gz
var stage2Compressed []byte

//go:embed 414a4c61-019f-48ba-934d-d5e91a29a878-T1071.001.exe.gz
...
func main() {
	...
	Endpoint.Say("Threat Actor: UNK_RobotDreams (Pakistan-aligned)")
	Endpoint.Say("=================================================================")
	Endpoint.Say("")
...
func test() {
	killchain := []KillchainStage{
		{
			ID:          1,
			Name:        "Malicious PDF Lure Preparation",
			Technique:   "T1204.002",
			BinaryName:  fmt.Sprintf("%s-T1204.002.exe", TEST_UUID),
			BinaryData:  stage1Compressed,
			Description: "Create PDF with embedded PowerShell trigger simulating Gulf Security Alert lure from fake Indian MEA",
		},
		{
			ID:          2,
			Name:        "PowerShell Staging and Payload Retrieval",
			Technique:   "T1059.001",
			BinaryName:  fmt.Sprintf("%s-T1059.001.exe", TEST_UUID),
			BinaryData:  stage2Compressed,
			Description: "Execute PowerShell download cradle to retrieve simulated Rust backdoor binary (T1059.001 + T1105)",
		},
		{
			ID:          3,
			Name:        "Azure Front Door C2 Communication",
			Technique:   "T1071.001",
			BinaryName:  fmt.Sprintf("%s-T1071.001.exe", TEST_UUID),
			BinaryData:  stage3Compressed,
			Description: "Establish HTTPS C2 channel via Azure Front Door CDN with encrypted system metadata (T1071.001 + T1573.001 + T1036.005)",
		},
	}
	...
	stageSeverity := "high"
	stageTactics := []string{"execution", "command-and-control", "defense-evasion"}
	...
	// Execute killchain
	Endpoint.Say("[*] Executing %d-stage UNK_RobotDreams attack killchain...", len(killchain))
	Endpoint.Say("")
	...
	for idx, stage := range killchain {
		...
		if exitCode == 126 || exitCode == 105 {
			...
			Endpoint.Say("")
			Endpoint.Say("EDR successfully blocked the UNK_RobotDreams attack at stage %d:", stage.ID)
			Endpoint.Say("  Technique: %s", stage.Technique)
	...
	Endpoint.Say("")
	Endpoint.Say("CRITICAL: Complete UNK_RobotDreams attack chain executed without prevention")
	Endpoint.Say("")
	...
	Endpoint.Say("  PDF lure delivered with embedded PowerShell trigger")
	Endpoint.Say("  PowerShell download cradle executed undetected")
	Endpoint.Say("  C2 communication via Azure Front Door CDN established")
	Endpoint.Say("  Domain fronting technique masked true C2 destination")
	Endpoint.Say("")
