...
#   GOOS=windows (default) → .exe extension, Authenticode signing
#   GOOS=linux             → no extension, signing skipped
#   GOOS=darwin            → no extension, ad-hoc codesign (if available)
#
...
# Binary extension: .exe for Windows, empty for Linux/macOS
...
validate_environment() {
    ...
    # Check signing tools (platform-specific)
    if [ "$GOOS" = "windows" ]; then
        ...
        if ! command -v codesign &> /dev/null; then
            print_warning "macOS codesign not available - ad-hoc signing will be skipped"
        fi
    fi
