...
### 7. Endpoint Validation Phase (MANDATORY)

**The agent MUST NOT consider a test complete until it has been deployed and validated on the target endpoint(s).** This phase catches false positives, SYSTEM context bugs, and runtime errors that only manifest on real systems.

#### Deployment Targets

| Platform | Deploy Path | SSH/SCP Host |
|----------|------------|--------------|
| Windows | `c:\F0\` | `win` |
| Linux | `/opt/f0/` | `debian` |
| macOS | `/opt/f0/` | `mac` |
...
1. **Copy the signed binary** to the target endpoint:
   ```bash
   # Windows test
...
2. **Execute the test** on the target endpoint and capture output:
   ```bash
   # Windows
   ssh win 'c:\F0\<uuid>.exe'

   # Linux
   ssh debian '/opt/f0/<uuid>'

   # macOS (remove quarantine first)
   ssh mac 'xattr -cr /opt/f0/<uuid> && /opt/f0/<uuid>'
   ```

3. **Review the output** for:
   - **Exit code correctness**: Does the exit code match what actually happened? (See Rule 8 — never trust a 126 without evidence)
   - **False positives**: Did a non-EDR error get classified as "blocked" (126) or "quarantined" (105)?
   - **SYSTEM context failures**: Did registry/schtasks/service operations fail due to running as SYSTEM? (See Rule 2)
   - **Runtime errors**: Missing dependencies, path issues, permission errors
   - **Stage execution**: For multi-stage tests, did all stages execute and report correctly?

4. **If errors or false positives are found**:
   - Fix the root cause in the source code
   - Rebuild and re-sign the binary
   - Re-deploy and re-test
   - Repeat until the test runs cleanly with correct exit codes

5. **The agent is DONE only when**:
   - The test binary executes successfully on the target endpoint
   - Exit codes accurately reflect what happened (no false positives)
   - All stages report correctly (for multi-stage tests)
   - The output log shows no unexpected errors

**IMPORTANT**: Deploy to ALL platforms the test targets. A Windows-only test needs validation on `win`. A cross-platform test needs validation on all applicable hosts.

### 8. Kill Chain Visualization (Optional)

After endpoint validation, optionally invoke `@kill-chain-diagram-builder` to generate a Cytoscape.js kill chain strip diagram. Recommended for multi-stage tests (3+ stages).

Output: `<test-dir>/kill_chain.html`

## F0RT1KA Framework Requirements

### Platform Selection (Autonomous)

When analyzing threat intelligence, **automatically determine the target platform** based on the threat:
...
- Copy `test_logger.go` (shared, NO build tag) from `sample_tests/multistage_template/`
- Copy `test_logger_<platform>.go` (platform constants + system info) from same location
- Windows tests: include `golang.org/x/sys` in go.mod
- Linux/macOS tests: do NOT include `golang.org/x/sys`

**Build commands per platform:**
- Windows: `GOOS=windows GOARCH=amd64 go build -o test.exe main.go test_logger.go test_logger_windows.go org_resolver.go`
- Linux: `GOOS=linux GOARCH=amd64 go build -o test main.go test_logger.go test_logger_linux.go org_resolver.go`
- macOS: `GOOS=darwin GOARCH=arm64 go build -o test main.go test_logger.go test_logger_darwin.go org_resolver.go`

**Code signing:**
- Windows: Authenticode via `osslsigncode` / `utils/codesign`
- Linux: Signing skipped (no Authenticode for ELF)
- macOS: Ad-hoc `codesign -s -` after build

**macOS deployment note:** Include `xattr -cr` quarantine removal in deployment instructions.

### Critical Rules
- **ALL binaries MUST be dropped to the platform LOG_DIR** - `C:\F0` (Windows) / `/tmp/F0` (Linux/macOS)
- **Simulation artifacts MUST be created in the platform ARTIFACT_DIR** - `c:\Users\fortika-test` (Windows) / `/home/fortika-test` (Linux) / `/Users/fortika-test` (macOS)
...
- **ALL tests MUST capture embedded binary stdout/stderr to file** - Use `io.MultiWriter` to output to both console AND save to `LOG_DIR/<binary>_output.txt`
...
These rules address recurring bugs found across multiple tests. Violating these will cause **false positive exit codes**.

#### Rule 1: NEVER Inject Blame Keywords Into Error Messages
Error wrappers must describe the operation that failed, NOT interpret WHY it failed. The `determineExitCode()` function pattern-matches on keywords like "access denied", "blocked", "prevented" — if you inject those into `fmt.Errorf()`, ANY error (disk full, path not found, etc.) will be misclassified as "EDR blocked" (exit 126).
...
**CRITICAL**: The `determineExitCode()` default fallback MUST be `StageError` (999), NEVER `StageBlocked` (126). See Rule 8.

#### Rule 2: Handle SYSTEM vs User Execution Context
Tests running via Prelude agent execute as SYSTEM. HKCU maps to `HKU\.DEFAULT` which lacks user-specific keys (Run, Environment). Always detect context and adapt:
...
**SYSTEM context also affects schtasks.exe**: `/SC ONLOGON /RL HIGHEST` fails as SYSTEM because Windows can't resolve a user SID for the UserId XML field. Use `/RU SYSTEM` instead of `/RL HIGHEST` when `isSystemContext()` returns true:

```go
if isSystemContext() {
    args = append(args, "/RU", "SYSTEM")  // No SID resolution needed
} else {
    args = append(args, "/RL", "HIGHEST") // User context — elevate
}
```

#### Rule 3: Use os.Stat() for Quarantine Detection, NOT Endpoint.Quarantined()
`Endpoint.Quarantined(filename, data)` calls `Pwd(filename)` which does `filepath.Join(cwd, filename)`. With absolute paths like `C:\F0\stage.exe`, this produces invalid doubled paths. Use direct `os.Stat()` instead:
...
#### Rule 4: Separate Benign Simulation Metrics from Critical Protection Metrics
When a test has both benign simulation steps (e.g., fake driver loading) and critical protection tests (e.g., EDR service tampering), track them with SEPARATE counters. Exit code must be determined by the CRITICAL metrics only:

```go
// ❌ WRONG — single counter mixes benign and critical
var totalAttempts, totalBlocked int

// ✅ CORRECT — separate counters
var driverBlocked bool           // benign simulation
var svcTamperAttempts int        // critical metric
var svcTamperBlocked int         // critical metric
var svcTamperSucceeded int       // critical metric
...
`sc.exe stop` on tamper-protected services may return empty output with non-zero exit code. Always handle unclear results:

```go
if stopErr != nil || containsAny(stopOutput, "access", "denied", "failed") {
    blocked++
} else if wasRunning {
    succeeded++ // actually stopped a running service
} else {
    blocked++ // empty/unclear output = treat as blocked
}
```

#### Rule 6: Use Windows Service Names, NOT Display Names
`sc.exe query` uses the registry service name (under `HKLM\SYSTEM\CurrentControlSet\Services\`), not the display name shown in services.msc. Using display names causes silent "not found" skips:

```go
// ❌ WRONG — display name, not service name
"CrowdStrike Falcon Sensor"

// ✅ CORRECT — actual service name
"CSFalconService"
```

#### Rule 7: Use Gzip Compression for Multi-Stage Embedded Binaries
Multi-stage orchestrators embed multiple signed PE binaries, producing 25-40MB executables. This causes agent download timeouts. ALWAYS compress stage binaries with gzip at build time and decompress in memory during extraction. NEVER use UPX or runtime packers — they trigger EDR heuristic detections (high entropy, unpacker stubs), poisoning test results with false positives for the packer rather than the technique.

```go
// ❌ WRONG — embeds raw PE (bloated orchestrator, agent download timeout)
//go:embed TEMPLATE-UUID-T1134.001.exe
var stage1Binary []byte

// ✅ CORRECT — embeds gzip-compressed PE (35% smaller orchestrator)
//go:embed TEMPLATE-UUID-T1134.001.exe.gz
var stage1Compressed []byte

// Decompress in memory before writing to disk
...
// ... then os.WriteFile(stagePath, binaryData, 0755)
```

**Build script**: Add `gzip -9 -k "$binary"` after signing, before embedding. Embed `.exe.gz` files. Clean up both `.exe` and `.exe.gz` after build.

#### Rule 8: NEVER Default to StageBlocked in determineExitCode()
In security test frameworks, never assume an unrecognized error means "blocked." The safest default is "error/unknown" (999) — let humans investigate rather than auto-declaring the endpoint secure. A false "PROTECTED" verdict is worse than an "ERROR" verdict because it gives false confidence that the EDR is working when it may not be.

```go
func determineExitCode(err error) int {
    if err == nil { return StageSuccess }
    errStr := err.Error()
    // Only EXPLICIT pattern matches should return blocked/quarantined
    ...
    // ❌ WRONG — causes false positive "EDR blocked" for ANY unrecognized error
    // return StageBlocked

    // ✅ CORRECT — unrecognized errors are test errors, not EDR actions
    return StageError
}
```

**Principle**: Exit code 126 (blocked) and 105 (quarantined) are *claims* about EDR behavior. They must only be returned when there is positive evidence (pattern-matched OS error strings) that the EDR actually acted. Everything else is an error (999).
...
### Multi-Stage Per-Stage ES Fan-Out (bundle_results.json)

**When to use**: ALL multi-stage tests (3+ techniques with sequential killchain execution). This is MANDATORY for multi-stage architecture.

Multi-stage tests MUST call `WriteStageBundleResults()` (defined in `test_logger.go`) to produce per-stage Elasticsearch documents. Without this, a 5-stage attack chain appears as a single row in the Executions table — you can't see which technique was blocked.

**How it works**: Each stage becomes an independent ES document via the same bundle fan-out pipeline used by cyber-hygiene tests. Zero agent or backend changes needed.

**Implementation pattern** (already in `TEMPLATE-UUID.go`):

1. **Initialize stage results array** before the execution loop:
```go
...
for i, stage := range killchain {
    stageResults[i] = StageBundleDef{
        ...
        Name:      stage.Name,
        Severity:  metadata.Severity, // from test metadata
...
2. **Update results as each stage completes** inside the loop:
```go
...
3. **Call `WriteStageBundleResults()` before EVERY exit point** (blocked, error, all-succeeded):
```go
WriteStageBundleResults(TEST_UUID, TEST_NAME, "intel-driven", subcategory, stageResults)
```

**Exit code normalization** (handled automatically by `WriteStageBundleResults`):
- Stage exit `0` (attack succeeded) → normalized to `101` (Unprotected in ES)
- Stage exit `126`/`105` (blocked) → kept as-is (Protected)
- Stage exit `999` (error) → kept as-is (Error)
- Skipped stages (default, never executed) → exit `0` with `skipped: true` (Inconclusive, excluded from Defense Score)

**Semantic mapping** (how bundle fields map for multi-stage):

| Field | Value |
|-------|-------|
| `control_id` | Technique ID (e.g., `T1105`) |
| `control_name` | Stage name (e.g., "Ingress Tool Transfer") |
| `validator` | `"Stage N"` (auto-derived from array index) |
| `category` | `"intel-driven"` |
| `compliant` | `true` if stage was blocked (endpoint defended) |

**Reference**: `tests_source/intel-driven/eafce2fc-75fd-4c62-92dc-32cabe5cf206/` (Tailscale, 5 stages).
...
3. **File Structure for Multi-Stage Tests**:
   ...
   ├── <uuid>.go                    # Main orchestrator (with platform build tag)
   ...
   ├── test_logger.go               # Shared logger (NO build tag, copy from template)
   ├── test_logger_<platform>.go    # Platform-specific logger (copy from template)
...
5. **Main Orchestrator Template Usage**:
   ...
   // Windows: use .exe extension | Linux/macOS: no extension
   //go:embed your-uuid-T1134.001.exe  // Windows
   //go:embed your-uuid-T1134.001      // Linux/macOS
   var stage1Binary []byte

   // Define killchain
   // Windows: BinaryName includes .exe | Linux/macOS: no extension
   killchain := []Stage{
       {
           ID:          1,
           Name:        "Access Token Manipulation",
           Technique:   "T1134.001",
           BinaryName:  fmt.Sprintf("%s-T1134.001.exe", TEST_UUID),  // Windows
           // BinaryName: fmt.Sprintf("%s-T1134.001", TEST_UUID),    // Linux/macOS
...
10. **Exit Code Logic for Multi-Stage**:
   ...
   - **MANDATORY**: Call `WriteStageBundleResults()` before every `Endpoint.Stop()` call (see template for pattern). Remaining stages default to "skipped" status.
