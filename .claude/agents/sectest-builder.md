...
## Workflow: Three-Phase Architecture
...
└── sectest-validation           → ES sync, endpoint deploy, git commit
```
...
### Phase 3 Failures
- **Endpoint validation false positive**: Fix source code → rebuild → re-sign → re-deploy (iterative loop)
