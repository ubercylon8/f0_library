...
## Step 5: Endpoint Validation (MANDATORY)

The test MUST be deployed and validated on the target endpoint(s). This catches false positives, SYSTEM context bugs, and runtime errors.
...
3. **Review output for**:
   - **Exit code correctness**: Does exit code match what actually happened?
   - **False positives**: Did a non-EDR error get classified as "blocked" (126)?
   - **SYSTEM context failures**: Registry/schtasks/service operations failing as SYSTEM?
   - **Runtime errors**: Missing dependencies, path issues, permission errors
   - **Stage execution**: For multi-stage, did all stages execute and report correctly?

4. **If errors found**: Fix source code → rebuild → re-sign → re-deploy → re-test

5. **Done when**:
   - Binary executes successfully on target endpoint
   - Exit codes accurately reflect what happened
   - All stages report correctly (multi-stage)
   - Output log shows no unexpected errors

**Deploy to ALL platforms the test targets.**
