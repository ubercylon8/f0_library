...
5. **Handle empty sc.exe output with corroboration, not assumption** — Tamper-protected services may return empty/unclear output on stop/config. Do NOT treat empty output alone as a block. Confirm with positive evidence — re-query the service and verify it is still running (i.e. the stop did not take effect). Only then classify as blocked. (See Rule 8.)
...
8. **Classify "blocked" only on positive evidence — never default to it** — A block code (126 `ExecutionPrevented`, 105/127 quarantine) must be returned ONLY when there is *affirmative* evidence of a protection action: a quarantine confirmed via `os.Stat`, an operation that normally succeeds in this context returning an OS-emitted denial, or a service still running after a stop attempt. **Absence of success is not evidence of a block.** Ambiguous, unrecognized, or benign-failure errors MUST map to 999 (`UnexpectedTestError`) or 0 — NEVER to a block code. A `default: return StageBlocked` (or any catch-all resolving unknown errors to 126) is a **banned anti-pattern**: it manufactures false "PROTECTED" verdicts that tell users their endpoint stopped an attack when nothing did. Common trap: an errno such as `ERROR_ACCESS_DENIED` on an object the test never created — e.g. opening a `\\.\Device` for a driver that was deliberately never loaded — is a benign "not present" outcome, not an EDR block. The objective is classification integrity: report what actually happened, never reassure on assumption.
